            ...
            <table id="warn-process-table" border="0" cellspacing="0" cellpadding="0" class="table-row2 nowrap" width="100%">
                ...
                <tbody>
                	<c:forEach var="detail" items="${data}" varStatus="st" >
                		<c:if test="${detail.riskName!=null && detail.riskName!='' }">
                			<tr>
                			<td>${st.index+1}</td>
                			<td>${detail.riskName }</td>
                			<td><input style="width: 200px;" type="text" detailid="${detail.id}" /></td>
                		</tr>
                		</c:if>
                	</c:forEach>
                </tbody>
...
<script src="<c:url value='/js/biz/warn_process_detail.js'/> "></script>
</html>
